...
function CheckOut() {
  ...
  const resData = location.state || JSON.parse(localStorage.getItem('checkoutData')); // Get data from state or local storage
  const [data, setData] = useState(null);

  useEffect(() => {
    if (resData) {
      setData(resData); // Store data in state if it exists
    }
  }, [resData]);
  ...
  const qrCodeUrl = `${window.location.origin}/parking/scannedcheckout/${encodeURIComponent(resEncrypt)}`;
