...
const RecentTransactions = ({ transactions, filterRecent, setFilterRecent, setShowAll }) => {
  const filteredRecentTransactions =
    filterRecent === "All"
      ? transactions
      : transactions.filter((transaction) => transaction.status === filterRecent);
      ...
      <div className="flex flex-col lg:flex-row p-0 h-min justify-between mb-4">
        <div className="dropdown">
          ...
          <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
            <li><a onClick={() => setFilterRecent("All")}>All</a></li>
            <li><a onClick={() => setFilterRecent("Paid")}>Paid</a></li>
            <li><a onClick={() => setFilterRecent("Unpaid")}>Unpaid</a></li>
            <li><a onClick={() => setFilterRecent("Canceled")}>Canceled</a></li>
          </ul>
        </div>

        {/* See All Button */}
        <button className="btn btn-primary bg-orange-600 mt-4 lg:mt-0" onClick={() => setShowAll(true)}>
          See All
        </button>
      </div>
